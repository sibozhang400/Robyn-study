...
# Rename the column 'revenue' to 'chapter_view'
colnames(dt_simulated_weekly)[colnames(dt_simulated_weekly) == 'revenue'] <- 'chapter_view'
colnames(dt_simulated_weekly)[colnames(dt_simulated_weekly) == 'tv_S'] <- 'ctv_S'

dt_simulated_weekly$chapter_view <- dt_simulated_weekly$chapter_view / 10

# Add 4 years to the DATE column in simulated data
dt_simulated_weekly$DATE <- as.Date(dt_simulated_weekly$DATE) + lubridate::years(4)
...
InputCollect <- robyn_inputs(
  ...
  dep_var = "chapter_view", # there should be only one dependent variable
  dep_var_type = "revenue", # "revenue" (ROI) or "conversion" (CPA)
  prophet_vars = c("trend", "season", "holiday"), # "trend","season", "weekday" & "holiday"
  prophet_country = "DE", # input country code. Check: dt_prophet_holidays
#   context_vars = c("competitor_sales_B", "events"), # e.g. competitors, discount, unemployment etc
  paid_media_spends = c("ctv_S", "ooh_S", "facebook_S", "search_S"), # mandatory input
  paid_media_vars = c("ctv_S", "ooh_S", "facebook_I", "search_clicks_P"), # mandatory.
...
hyperparameters <- list(
  ...
  ctv_S_alphas = c(0.5, 3),
  ctv_S_gammas = c(0.3, 1),
  ctv_S_thetas = c(0.3, 0.8),
...
df <- data.frame(
  channel = rep(c('CTV', 'OOH', 'Paid Digital'), 2),
#   uplift = c(11, 4.2, 7, 10, 4.6, 8),
  uplift = c(11, 4.2, 11.2, 6.5, 6.2, 6),
  year = c(rep('FY23', 3), rep('FY24', 3))
)

# Convert uplift to percentage
df$uplift <- df$uplift / 100

# Create the bar plot with adjustable width and height
ggplot(df, aes(x = channel, y = uplift, fill = year)) +
    geom_bar(stat = 'identity', position = 'dodge') +
    geom_text(aes(label = scales::percent(uplift)), 
                        position = position_dodge(width = 0.9), 
                        vjust = -0.25, size=5, family="Arial") +
    scale_y_continuous(labels = scales::percent) +
    labs(x = "Channel", y = "Uplift Effect", fill = "Year") +
    theme_minimal() + 
    theme(
        ...
        plot.title = element_text(size = 18, family = "Arial", 
                                  face = "bold",hjust = 0.5) # Add title styling
    ) +
    ggtitle("Uplift Effect by Channel and Fiscal Year")

# Save the plot with specified width and height
ggsave(paste(robyn_directory,"/", "uplift_effect_plot2.png", sep=""), width = 10, height = 6)
...
# Create a data frame with your data
# df_total <- data.frame(
#   total_spend = c(6,000,000, 6,000,000),
#   total_cv = c(8,440,000, 12,700,000),
#   category = c('initial', 'optimal')
# )

# df_channel <- data.frame(
#   channel = rep(c('CTV', 'OOH', 'Paid Digital'), 2),
#   uplift = c(2,250,000, 1,100,000, 2,150,000, 6,100,000, 1,550,000, 4,350,000),
#   category = c('intial', 'intial', 'intial', 'optimal', 'optimal', 'optimal')
# )

# # Create the bar plot with adjustable width and height
# ggplot(df_total, aes(x = channel, y = uplift, fill = year)) +
#     geom_bar(stat = 'identity', position = 'dodge') +
#     geom_text(aes(label = scales::percent(uplift)), 
#                         position = position_dodge(width = 0.9), 
#                         vjust = -0.25, size=5, family="Arial") +
...
# # Save the plot with specified width and height
# ggsave(paste(robyn_directory,"/", "uplift_effect_plot.png", sep=""), width = 10, height = 6)

# Create a sample data frame
df_total <- data.frame(
  category = c("total spend", "total response", "total spend", "total response"),
  value = c(6, 8.44, 6, 12.7),
  group = rep(c("initial", "optimal"), each = 2)
)
...
# Create the bar plot with adjustable width and height
ggplot(df_total, aes(x = category, y = value, fill = group)) +
    geom_bar(stat = 'identity', position = 'dodge') +
    ...
    ggtitle("Initial vs Optimal Budget Allocation")